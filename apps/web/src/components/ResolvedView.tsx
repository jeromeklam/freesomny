...
function SourceTag({ source }: { source: string }) {
  return (
    <span className="inline-flex items-center px-1.5 py-0.5 text-xs font-medium bg-gray-700 text-gray-300 rounded">
      [{source}]
    </span>
  )
}
            ...
            <tbody>
              {resolved.headers.map((header, i) => (
                <tr key={i} className="border-b border-gray-700/50 last:border-0">
                  <td className="px-3 py-2 font-mono text-gray-300">{header.key}</td>
                  <td className="px-3 py-2 font-mono text-gray-300 truncate max-w-xs">
                    {header.value}
                  </td>
