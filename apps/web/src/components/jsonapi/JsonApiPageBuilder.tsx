    ...
    <div className="p-4 space-y-4">
      <div className="flex items-center gap-4">
        <div className="flex-1">
          <label className="block text-xs text-gray-400 mb-1">{t('jsonapi.page.offset')}</label>
          <input
            type="text"
            value={state.offset}
            onChange={(e) => onChange({ ...state, offset: e.target.value })}
            placeholder={t('jsonapi.page.offsetPlaceholder')}
            className="w-full px-2 py-1.5 bg-gray-800 border border-gray-700 rounded text-sm font-mono focus:outline-none focus:border-blue-500"
          />
        </div>
        <div className="flex-1">
          <label className="block text-xs text-gray-400 mb-1">{t('jsonapi.page.limit')}</label>
          <input
            type="text"
            value={state.limit}
